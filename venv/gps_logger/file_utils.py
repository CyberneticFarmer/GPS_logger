...
def create_new_file(base_dir):
    now = datetime.now()
    day_dir = "C:/Users/reidb/PycharmProjects/250609_KML_norgeskart/venv/251220_webvisning/tracks/" + now.strftime("%Y-%m-%d")
...
def write_geojson(path, coords, start_time, track_name="Track 1"):
    geojson = {
        "type": "FeatureCollection",
        "features": [
            {
...
def update_track_index(
    ...
    if any(t["id"] == track_id for t in index_data[date_str]):
        ...
        return
    track_path = "./tracks/" + date_str + "/" + track_id.replace("-","") + ".js" # ./tracks/2025-12-21\\160644.js"
    # Append new entry
