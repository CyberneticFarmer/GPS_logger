...
def calculate_path_length(coords):
    ...
    for i in range(1, len(coords)):
        total_distance += haversine_distance(coords[i][0], coords[i][1],coords[i-1][0] ,coords[i-1][1] )
        #print(total_distance)
    return total_distance/100


def check_muli_emptying(rov):
    lat, lng = rov["coor"][-1]
    dist = haversine_distance(lat, lon, barn_coor[0], barn_coor[1])
    if dist < 1000 :
        return true
    else:
        return false
